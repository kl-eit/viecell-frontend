  ...
  return (
    <SectionBlock>
      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="lg:col-span-6 col-span-12 gap-2.5">
          {faqsData.faqs?.map((item, index) => (
            <div
              key={item.id}
              className={`px-5 rounded-[20px] border-b border-neutral-400/50 flex flex-col transition-colors duration-300 ${
                openIndex === index ? "bg-lime-100" : ""
              }`}
            >
              <button
                onClick={() => toggleIndex(index)}
                className="w-full py-8 flex justify-between items-center gap-4"
              >
