                ...
                return (
                  <li key={levelIndex} className="font-['Roboto_Condensed'] text-sm">
                    {hasChildren ? (
                              ...
                              <ul className="pt-1 space-y-1 text-sm">
                                {item.children?.map((subMenu, index) => (
                                  <li key={index} className="px-2">
                                    <div className="flex items-center justify-between" onClick={() => setActive(active === index ? null : index)}>
                                      <Link href={subMenu?.url} className="pl-6 flex items-center px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group">
                                        {subMenu?.title}
                                      </Link>
                                      <svg
                                    ...
                                    <AnimatePresence>
                                      {active === index &&
                                        subMenu.children?.map((childItem, idx) => (
                                          <motion.div key={idx} initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                                            <Link href={childItem?.url} className="pl-10 flex items-center px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group">
                                              {childItem?.label}
                                            </Link>
                                          </motion.div>
                                        ))}
                      ...
                      <Link className="flex items-center px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group" href={`${item?.url}`}>
                        {item?.label}
                      </Link>
                    )}
                  </li>
                ...
                <>
                  ...
                  <NavigationMenuContent className="px-0 py-1">
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-2 p-4 w-5xl bg-white mt-2">
                      {item.children.map((submenu, index) => (
                        <div key={index}>
                          <h6 className="font-extrabold font-['Roboto_Condensed']">{submenu.title}</h6>
                          <ul className="mt-2.5 grid gap-1 text-[14px]">
                            {submenu.children.map((child, idx) => (
                              <li key={idx}>
